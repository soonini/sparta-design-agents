...
## 다음 작업: 사내 평가 체크리스트 통합

사용자가 사내 디자인 평가 체크리스트를 기존 시스템에 통합 요청함. **계획 단계에서 중단됨** (Plan Mode 진입 후 서브에이전트 rate limit으로 설계 미완).

### 요구사항

사내 평가 체크리스트를 제작/검수 시 강력 반영. 특히 검수 시 **정량적 기준**으로 상세 정의.

### 사내 평가 체크리스트 원문

**퀄리티**
- [필수] 가독성 및 접근성: 모바일/다양한 디바이스에서 콘텐츠가 명확히 보이는가
- [필수] 레이아웃 완성도: 그리드, 간격, 타이포 마감이 디테일한가
- [중요] 콘텐츠 몰입도: 디자인이 콘텐츠 목적을 방해하지 않고 집중력을 높여주는가

**브랜드**
- [필수] 가이드라인 준수: 로고, 컬러, 폰트 사용 규정 준수 (→ 기존 32항목과 중복)
- [중요] 일관된 경험 제공: 다른 채널/매체 콘텐츠와 이질감 없는가
- [😎] 혁신성: 브랜드 확장 새 표현형식/기법 시도

**그로스**
- [😎] 지표 개선: 관련 지표 상승 가능성
- [😎] 생산성 개선: 재사용 가능 템플릿/모듈 제작
...
### 설계 방향 (탐색 단계에서 파악한 내용)

1. **별도 레이어로 운영**: 기존 32항목(기술 준수) + 사내 평가(종합 퀄리티)를 이원 체계로 운영
2. **정량적 기준 정의 필요**: 각 항목에 AI가 판정 가능한 측정 수치/조건 필요
   - 예: 최소 폰트 크기, 명도 대비 비율, 터치 영역 크기, 그리드 정렬 등
3. **수정 대상 파일 4개**:
   - `skills/sparta-review-checklist.md` — 사내 평가 섹션 추가 + 정량 기준 + 등급 정의
   - `agents/sparta-design-reviewer.md` — 워크플로우에 사내 평가 단계 추가
   - `agents/sparta-design-creator.md` — 셀프 검수에 필수 항목 반영
   - `skills/sparta-design-guideline.md` — 제작 워크플로우에 퀄리티 기준 참조 추가
4. **검수 리포트에 등급 포함**: 기존 PASS/FAIL 판정 + 사내 등급(🔴/🟡/🟢/😎) 병기

### 다음 세션에서 할 일

1. 위 설계 방향을 구체화 (정량적 기준 상세 정의)
2. `skills/sparta-review-checklist.md`에 사내 평가 섹션 추가
3. 에이전트 2개 + 가이드라인 1개 수정
4. CLAUDE.md, HANDOVER.md 갱신 + 커밋
