...
> 이 명령어는 에이전트, 스킬 파일을 Claude Code가 인식하는 폴더에 복사합니다.
...
| 가독성·접근성 | 4개 | CRITICAL | 폰트 크기, 명도 대비, CTA 터치 영역, Safe Area |
| 레이아웃 완성도 | 5개 | CRITICAL | 그리드 정렬, 간격 일관성, 여백 균형 |
| 타이포그래피 | 6개 | HIGH | 폰트, 크기, 줄간격, 자간 등 |
| 그래픽 | 9개 | HIGH | 아이콘, 일러스트, 사진 규격 등 |
| 콘텐츠 몰입도 | 5개 | HIGH | 시각 진입점, 정보 밀도, CTA 식별성 |
| 브랜드 일관성 | 4개 | HIGH | 톤앤매너, 컬러·타이포 패턴 일관성 |
| 레이아웃 | 5개 | MEDIUM | 포맷 비율, 정렬, 텍스트 영역 등 |

**신호등 등급:**

| 등급 | 조건 |
|------|------|
| 🔴 **미달** | CRITICAL 1건 이상 FAIL — 게시 불가 |
| 🟡 **기본** | CRITICAL 0건, HIGH 1건 이상 FAIL — 개선 권장 |
| 🟢 **우수** | CRITICAL + HIGH 전부 PASS — 품질 기준 충족 |
...
│
├── skills/                              # 브랜드 지식 + 슬래시 커맨드
│   ├── sparta-brand-components/SKILL.md #   컬러, 로고, 폰트 등 디자인 토큰
│   ├── sparta-design-guideline/SKILL.md #   디자인 제작 가이드라인
│   ├── sparta-review-checklist/SKILL.md #   32개 검수 체크리스트
│   ├── sparta-feedback-loop/SKILL.md    #   피드백 수집/분석 규칙
│   ├── sparta-design/SKILL.md           #   /sparta-design 커맨드
│   ├── sparta-review/SKILL.md           #   /sparta-review 커맨드
│   ├── sparta-feedback/SKILL.md         #   /sparta-feedback 커맨드
│   └── sparta-learn/SKILL.md            #   /sparta-learn 커맨드
│
├── feedback/                            # 피드백 저장소
...
| 증상 | 해결 방법 |
|------|----------|
| 명령어 입력 시 반응이 없음 | Claude Code가 실행 중인지 확인. `~/.claude/skills/` 폴더에 파일이 복사되었는지 확인 |
