...
3. **sparta-review-checklist** — 셀프 검수 시 사용하는 50개 체크리스트 (CRITICAL 21 + HIGH 24 + MEDIUM 5) → 신호등 등급
...
CRITICAL → HIGH 순서로 확인하여 신호등 등급을 예측한다.

#### 셀프 검수 절차

1. **CRITICAL 확인** (21항목: 로고 7 + 컬러 5 + 가독성 4 + 레이아웃 완성도 5):
   - CSS 변수로 색상이 올바르게 참조되는가?
   - 로고 배치, 크기, 클리어 스페이스가 규격에 맞는가?
   - Body ≥ 14px, Head ≥ 24px, 대비 ≥ 4.5:1(본문) / ≥ 3:1(대형), CTA ≥ 44×44px
   - 4px 그리드 정렬, 간격 편차 ≤ 4px, 좌우 마진 차이 ≤ 8px

2. **HIGH 확인** (24항목: 타이포 6 + 그래픽 9 + 몰입도 5 + 브랜드 일관성 4):
   - 서체, Weight, Line Height가 위계에 맞는가?
   - Frame/Pattern/Tag 규격이 준수되는가?
   - 시각 진입점, 정보 밀도, CTA 식별성이 적절한가?
   - 톤앤매너, 컬러·타이포 패턴이 일관된가?

3. **수정**: FAIL 항목 발견 시 코드를 수정하고 재확인

4. **결과 보고**: 셀프 검수 결과를 사용자에게 보고

```markdown
### 셀프 검수 결과
- CRITICAL: 0건 FAIL / 21항목 → ✓
- HIGH: 0건 FAIL / 24항목 → ✓
- MEDIUM: 0건 FAIL / 5항목
- **예상 등급: 🟢 우수** (정식 등급은 /sparta-review에서 확정)
```
